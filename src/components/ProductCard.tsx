...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div 
      onClick={goToProduct}
      className="group bg-[var(--card)] border border-(--card-border) rounded-2xl p-4 hover:border-[var(--primary)]/50 transition-all duration-300 relative overflow-hidden block cursor-pointer shadow-sm hover:shadow-xl"
    >
      {product.badge && (
      ...
      <button 
        ...
        className={`absolute top-3 right-3 p-2 rounded-full bg-[var(--background)]/80 backdrop-blur-sm transition-all z-10 border border-(--card-border) hover:scale-110 ${isWishlisted ? 'text-red-500 border-red-500/30' : 'text-[var(--muted)] hover:text-red-500'}`}
      >
        <Heart size={14} fill={isWishlisted ? "currentColor" : "none"} />
      </button>
      
      <div className="aspect-square bg-[var(--input)] rounded-xl mb-4 flex items-center justify-center group-hover:scale-105 transition-transform duration-500 overflow-hidden border border-(--card-border)">
